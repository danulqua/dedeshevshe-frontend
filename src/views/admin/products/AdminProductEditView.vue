<template>
  <h2>Редагування продукту #{{ productId }}</h2>

  <ProgressSpinner v-if="isFetching" />
  <div v-else class="mt-4 flex flex-column gap-4 form">
    <div>
    ...
    <div>
      <label for="description" class="block text-900 font-medium mb-2">Опис</label>
      <PTextarea
        id="description"
        v-model="description"
        name="description"
    ...
    <div>
      <label for="shopId" class="block text-900 font-medium mb-2">Супермаркет</label>
      <PDropdown
        v-model="shopId"
        input-id="shopId"
    ...
    <div>
      <label for="url" class="block text-900 font-medium mb-2">Посилання</label>
      <PInputText
        id="url"
        v-model="url"
        name="url"
    ...
    <div>
      <label for="price" class="block text-900 font-medium mb-2">Ціна</label>
      <PInputNumber
        v-model="price"
        input-id="price"
        name="price"
    ...
    <div class="flex flex-column gap-2">
      ...
      <div v-if="isDiscount" class="flex flex-column gap-2">
        <div>
          <label for="discount.value" class="block text-900 font-medium mb-2"
        ...
        <div>
          <label for="discount.oldPrice" class="block text-900 font-medium mb-2">Стара ціна</label>
          <PInputNumber
            v-model="oldPrice"
    ...
    <div>
      ...
      <div v-if="isSolid">
        ...
        <PInputNumber
          ...
          :use-grouping="false"
        />
        <ErrorMessage name="weight" as="small" class="p-error">{{ errors.weight }}</ErrorMessage>
      </div>
      ...
      <div class="flex flex-column gap-2">
        ...
        <img
          ...
          class="w-12rem h-12rem shadow-2 border-round"
    ...
    <div>
      <label for="status" class="block text-900 font-medium mb-2">Статус</label>
      <PDropdown
        v-model="status"
        input-id="status"
        :options="statuses"
    ...
    <div class="flex gap-2">
      <PButton
        ...
        @click="submit"
      />
      <RouterLink :to="{ name: 'adminProducts' }">
        <PButton label="Скасувати" outlined />
      </RouterLink>
    </div>
  </div>
</template>
...
import type { Discount, ProductStatus } from '@/api/types/product';
...
watch([isSolid, isFetching], ([value, newIsFetching], [, oldIsFetching]) => {
  if (oldIsFetching && !newIsFetching) return;
...
watch([isDiscount, isFetching], ([value, newIsFetching], [, oldIsFetching]) => {
  if (oldIsFetching && !newIsFetching) return;
...
const getProductData = async () => {
  try {
    ...
    const product = await productsService.getProduct(productId.value);
    ...
    isDiscount.value = !!product.discount;
    discountValue.value = product.discount || null;
    oldPrice.value = product.oldPrice || null;
    isSolid.value = !!product.weight;
    volume.value = product.volume || null;
    weight.value = product.weight || null;
    imageId.value = product.imageId || null;
    imageUrl.value = product.image?.url || null;
...
const submit = handleSubmit(async (values) => {
  try {
    ...
    await productsService.updateProduct(productId.value, {
      ...values,
